...
final class YouTubeDataService {
    ...
    func myChannel(accessToken: String) async throws -> YouTubeChannelSnapshot {
        ...
        guard let item = (root["items"] as? [[String: Any]])?.first,
            ...
            throw YouTubeUploadError(message: "Für dieses Google-Konto wurde kein YouTube-Kanal gefunden.")
        }
        let stats = item["statistics"] as? [String: Any] ?? [:]
        let identity = ChannelIdentity(
            id: id,
            name: snippet["title"] as? String ?? "YouTube-Kanal",
            description: snippet["description"] as? String ?? "",
            primaryTopic: "",
            language: snippet["defaultLanguage"] as? String ?? "de"
        )
        return .init(
            identity: identity,
            uploadsPlaylistID: uploads,
            subscriberCount: Int(stats["subscriberCount"] as? String ?? "") ?? 0,
            viewCount: Int(stats["viewCount"] as? String ?? "") ?? 0,
            videoCount: Int(stats["videoCount"] as? String ?? "") ?? 0
        )
    }
    ...
    func channelVideos(uploadsPlaylistID: String, accessToken: String, maximum: Int = 80) async throws -> [ChannelVideoProfile] {
        ...
        while ids.count < maximum {
            ...
            c.queryItems = [
                ...
                .init(name: "maxResults", value: String(min(50, maximum - ids.count)))
            ]
            ...
            for item in root["items"] as? [[String: Any]] ?? [] {
                if let id = (item["contentDetails"] as? [String: Any])?["videoId"] as? String { ids.append(id) }
            }
    ...
    /// Quota-conscious discovery: one mostPopular call plus at most one search.list call
    /// built from the strongest Channel-DNA terms. This avoids a generic trends wall while
    /// still keeping YouTube API cost predictable on each manual refresh.
    func discover(regionCode: String, dna: ChannelDNA, accessToken: String) async throws -> [Opportunity] {
        let popularItems = try await popular(regionCode: regionCode, dna: dna, accessToken: accessToken)
        let terms = Array(dna.topicKeywords.prefix(3))
        guard !terms.isEmpty else { return popularItems }

        let searched = try await searchRelevant(
            terms: terms,
        ...
        for item in popularItems + searched {
            if let old = byID[item.id] {
                byID[item.id] = old.score >= item.score ? old : item
            } else {
                byID[item.id] = item
            }
        }

        let ranked = byID.values.sorted { lhs, rhs in
            if lhs.score == rhs.score { return lhs.publishedAt > rhs.publishedAt }
            return lhs.score > rhs.score
        }
        let relevant = ranked.filter { $0.relevance >= 0.12 || $0.momentum >= 0.78 }
        return Array((relevant.isEmpty ? ranked : relevant).prefix(80))
    }
    ...
    func popular(regionCode: String, dna: ChannelDNA, accessToken: String) async throws -> [Opportunity] {
        ...
        let root = try await object(c.url!.absoluteString, token: accessToken)
        return opportunities(from: root["items"] as? [[String: Any]] ?? [], dna: dna)
    }
    ...
    private func searchRelevant(
        ...
        let after = ISO8601DateFormatter().string(from: Date().addingTimeInterval(-21 * 24 * 3600))
        let query = terms.map { "\"\($0)\"" }.joined(separator: "|")
        c.queryItems = [
            ...
            .init(name: "publishedAfter", value: after),
            .init(name: "regionCode", value: regionCode)
        ]
        let shortLanguage = language.components(separatedBy: "-").first ?? language
        if !shortLanguage.isEmpty { c.queryItems?.append(.init(name: "relevanceLanguage", value: shortLanguage)) }
        let root = try await object(c.url!.absoluteString, token: accessToken)
        let ids = (root["items"] as? [[String: Any]] ?? []).compactMap { item -> String? in
            (item["id"] as? [String: Any])?["videoId"] as? String
        }
        guard !ids.isEmpty else { return [] }
        return try await opportunityDetails(ids: ids, dna: dna, accessToken: accessToken)
    }

    private func opportunityDetails(ids: [String], dna: ChannelDNA, accessToken: String) async throws -> [Opportunity] {
        var all: [Opportunity] = []
        for start in stride(from: 0, to: ids.count, by: 50) {
            ...
            let root = try await object(c.url!.absoluteString, token: accessToken)
            all.append(contentsOf: opportunities(from: root["items"] as? [[String: Any]] ?? [], dna: dna))
        }
        return all.sorted { $0.score > $1.score }
    }

    private func opportunities(from items: [[String: Any]], dna: ChannelDNA) -> [Opportunity] {
        ...
        return items.compactMap { item in
            ...
            let viewsPerHour = Double(views) / ageHours
            let momentum = min(1, max(0, (log10(max(10, viewsPerHour)) - 1.7) / 3.6))
            return Opportunity(
                id: id,
                videoID: id,
                ...
                viewCount: views,
                durationSeconds: Self.duration((item["contentDetails"] as? [String: Any])?["duration"] as? String ?? "PT0S"),
                relevance: dnaEngine.relevance(of: text, to: dna),
                momentum: momentum
            )
    ...
    func playlists(accessToken: String) async throws -> [YouTubePlaylist] {
        ...
        return (root["items"] as? [[String: Any]] ?? []).compactMap { item in
            guard let id = item["id"] as? String,
                  let title = (item["snippet"] as? [String: Any])?["title"] as? String else { return nil }
            return .init(id: id, title: title)
        }
    }

    func add(videoID: String, toPlaylist playlistID: String, accessToken: String) async throws {
        var r = URLRequest(url: URL(string: "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet")!)
        r.httpMethod = "POST"
        ...
        for start in stride(from: 0, to: ids.count, by: 50) {
            ...
            for item in root["items"] as? [[String: Any]] ?? [] {
                ...
                out.append(.init(
                    ...
                    publishedAt: Self.date(s["publishedAt"] as? String) ?? Date(),
                    viewCount: Int(stats["viewCount"] as? String ?? "") ?? 0,
                    likeCount: Int(stats["likeCount"] as? String ?? "") ?? 0,
                    commentCount: Int(stats["commentCount"] as? String ?? "") ?? 0,
                    durationSeconds: Self.duration((item["contentDetails"] as? [String: Any])?["duration"] as? String ?? "PT0S"),
    ...
    private func object(_ address: String, token: String) async throws -> [String: Any] {
        var r = URLRequest(url: URL(string: address)!)
    ...
    private static func duration(_ value: String) -> Double {
        ...
        func g(_ i: Int) -> Double {
            let r = m.range(at: i)
            guard r.location != NSNotFound, let rr = Range(r, in: value) else { return 0 }
            return Double(value[rr]) ?? 0
        }
...
final class YouTubeAnalyticsService {
    ...
    func summary(days: Int = 28, accessToken: String) async throws -> AnalyticsSummary {
        ...
        c.queryItems = [
            ...
            .init(name: "metrics", value: "views,estimatedMinutesWatched,averageViewDuration,subscribersGained")
        ]
        ...
        return .init(
            views: (row.indices.contains(0) ? row[0] as? NSNumber : nil)?.intValue ?? 0,
            estimatedMinutesWatched: (row.indices.contains(1) ? row[1] as? NSNumber : nil)?.doubleValue ?? 0,
            averageViewDuration: (row.indices.contains(2) ? row[2] as? NSNumber : nil)?.doubleValue ?? 0,
            subscribersGained: (row.indices.contains(3) ? row[3] as? NSNumber : nil)?.intValue ?? 0
        )
    }
}
