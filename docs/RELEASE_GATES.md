...
Hinweis: **Capture = FAIL** bleibt bewusst bestehen: Der Canonical-Gate verlangt Camera/Mic/Screen/System-Audio-Capture. Inzwischen sind alle vier Capture-Arten als gemeinsame Core-Verträge modelliert, Kamera-/Mikrofon-Hardware und macOS-Berechtigungen werden nativ geprüft, Screen/Systemaudio sind an die Screen-Recording-Freigabe gebunden, die Studio-Oberfläche zeigt den realen Bereitschaftsstatus und das App-Bundle enthält die erforderlichen Camera-/Microphone-Usage-Descriptions. `Build/audit_capture.py` sichert diese Grundlage in der Canonical-CI. Für PASS fehlen weiterhin die echten Aufnahme-Pipelines inklusive Datei-Erzeugung, Stop/Fehlerbehandlung und Übernahme der aufgezeichneten Medien in den autorisierten Projekt-Workspace für alle vier Pfade.
