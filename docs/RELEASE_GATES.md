...
Hinweis: **Installer = PASS** und **Clean-Machine E2E = PASS** sind durch den erfolgreichen Canonical-CI-Lauf auf einem frischen, versionsgepinnten macOS-26-Runner mit Xcode 26.6 belegt. Der Paketierer baut Blackstock und den CI-only E2E-Helper getrennt für arm64 und x86_64 und vereinigt beide Slices vor dem Codesigning zu Universal-2-Binaries. Das erzeugte `.pkg` wird installiert; die Canonical-CI prüft beide Architekturen der tatsächlich installierten Binaries mit `lipo`, danach Bundle-Metadaten und Codesign, startet die installierte App und führt anschließend den Creator-Loop direkt aus `/Applications/Blackstock.app/Contents/Helpers/BlackstockE2ESmoke` aus. Der Helper erzeugt reales lokales AV-Testmaterial, rendert und validiert es inklusive professioneller Audio-QC, durchläuft Publishing mit gemockter externer HTTP-Grenze, verifiziert Upload-Journaling, Analytics, Growth-Learning und den exakten Persistenz-Roundtrip. Der E2E deckte dabei einen realen Zeitpräzisionsfehler in der Growth-Persistenz auf; Schema v4 speichert aktuelle Zeitstempel verlustfrei relativ zur Foundation-Referenzepoche und migriert v1/v2-ISO-8601 sowie v3-Unix-Zeitstempel weiterhin deterministisch. `Build/audit_clean_machine.py` verhindert, dass dieser installierte E2E-Vertrag unbemerkt aus der Canonical-CI verschwindet.
...
Hinweis: **Updater = FAIL** bleibt absichtlich bestehen. Manifest-Signatur, HTTPS-Pflicht, SHA-256-Paketprüfung und Developer-ID-Installer-Teamprüfung sind implementiert. Direkt vor der Übergabe an den macOS-Installer werden SHA-256 und Developer-ID-Installer-Team außerdem unmittelbar erneut geprüft; erst nach diesem Installations-Preflight wird die Verifikationsevidenz aktualisiert und das Paket geöffnet. Zusätzlich protokolliert Blackstock eine versionierte lokale In-App-Evidenzkette: akzeptiertes signiertes Manifest mit Source-Commit → verifiziertes Paket/Installer-Team → tatsächlich geöffneter macOS-Installer → nach Installation gestarteter exakter Manifest-Ziel-Build aus demselben signierten Source-Commit inklusive SHA-256 des gestarteten Executables. `Build/audit_updater_evidence.py` und `Build/validate_in_app_update_evidence.py` sichern diesen Vertrag. Für PASS fehlen weiterhin die reale Produktions-Endpoint-Konfiguration, das tatsächlich Developer-ID-signierte/notarisierte Release und ein vollständiger echter In-App-Versionswechsel.
