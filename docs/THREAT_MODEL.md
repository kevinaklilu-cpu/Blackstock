...
- Importierte Desktop-OAuth-Konfiguration persistiert ein optionales Desktop-`client_secret` ausschließlich im macOS-Keychain und nie in UserDefaults, Projektdateien oder Logs.
...
- Dasselbe erzeugte Paket wird per gepinntem GitHub-Artefakt-Handoff und SHA-256-Bindung auf einen nativen Intel-Runner übertragen; dort werden Installation, App-Launch und vollständiger Creator-Loop erneut ausgeführt. Ein separater Intel-Neubuild zählt ausdrücklich nicht als Ersatz für diesen Cross-Architecture-Nachweis.
- Nach der Installation erzeugen Apple-Silicon- und Intel-Runner jeweils einen maschinenlesbaren Release-Fingerprint aus Bundle-ID, Version, Build, Source-Commit, SHA-256 des exakten Installer-Pakets, SHA-256 des tatsächlich installierten Universal-2-Executables und macOS-Installer-Receipt. Bis auf die bewusst unterschiedliche Runner-Architektur müssen diese Identitätsfelder exakt übereinstimmen; beide Fingerprints müssen zusätzlich denselben SHA-256 tragen wie das auf Intel erneut heruntergeladene und vor Installation verifizierte Paket. Damit wird belegt, dass auf beiden Architekturen derselbe Installer und derselbe installierte Release ausgeführt wurden.
