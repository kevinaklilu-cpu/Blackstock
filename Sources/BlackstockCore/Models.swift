...
public enum VideoFormat: String, Codable, Sendable, CaseIterable {
    ...
    public static func infer(durationSeconds: Int) -> VideoFormat {
        durationSeconds <= 180 ? .short : .longform
    }
}
...
public struct TrendSignal: Codable, Identifiable, Sendable, Equatable {
    ...
    public init(video: VideoMetric, reasons: [TrendReason], recommendedFormat: VideoFormat, rankValue: Double, topic: String) {
        self.video = video; self.reasons = reasons; self.recommendedFormat = recommendedFormat; self.rankValue = rankValue; self.topic = topic
    }
}
...
public struct AnalyticsSnapshot: Codable, Sendable, Equatable {
    ...
    public init(views: Int = 0, watchTimeMinutes: Double = 0, impressions: Int = 0, clickThroughRate: Double? = nil, averageViewDurationSeconds: Double? = nil) {
        self.views = views; self.watchTimeMinutes = watchTimeMinutes; self.impressions = impressions; self.clickThroughRate = clickThroughRate; self.averageViewDurationSeconds = averageViewDurationSeconds
    }
}
