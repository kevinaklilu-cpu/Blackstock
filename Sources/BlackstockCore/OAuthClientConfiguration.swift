...
public enum OAuthClientConfigurationError: Error, Equatable, Sendable {
    case invalidJSON
...
public struct OAuthClientConfiguration: Sendable, Equatable {
    ...
    public static func parseGoogleDesktopJSON(
        _ data: Data
        ...
        let id = installed.clientID.trimmingCharacters(
    ...
    public static func preferredClientID(
        bundled: String,
        imported: String
    ) -> String {
        let importedValue = imported.trimmingCharacters(
            in: .whitespacesAndNewlines
        )
        if !importedValue.isEmpty {
            return importedValue
        }
        return bundled.trimmingCharacters(
            in: .whitespacesAndNewlines
        )
    }
}
